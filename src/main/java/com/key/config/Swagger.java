...
public class Swagger {
    ...
    public Docket createDocket(){
        ...
        accessTokenBuilder.name("authorization").description("自测时动态传入AccessToken入口(非必要参数)")
                .modelRef(new ModelRef("String")).parameterType("header").required(false);

        refreshTokenBuilder.name("refreshToken").description("自测时动态传入RefreshToken入口(非必要参数)")
                .modelRef(new ModelRef("String")).parameterType("header").required(false);
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                .apiInfo(apiInfo())
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.key.controller"))
    ...
    private ApiInfo apiInfo(){
        return new ApiInfoBuilder().title("吃货宝智能点餐系统后台管理-17206127徐雨轩")
                .description("后端接口文档")
                .termsOfServiceUrl("")
                .version("1.0")
                .build();
